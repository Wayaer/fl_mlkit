...
public class FlCamera: NSObject, FlutterTexture, AVCaptureVideoDataOutputSampleBufferDelegate {
    // 链接相机用的
    ...
    var _flCameraEvent: FlCameraEvent
    ...
    init(_ event: FlCameraEvent, _ registry: FlutterTextureRegistry) {
        _registry = registry
        _flCameraEvent = event
        super.init()
    }

    func initCamera(_ resolution: String, _ cameraId: String, _ result: @escaping FlutterResult, _ captureOutputCallBack: ((_ buffer: CMSampleBuffer) -> Void)?) {
    ...
    override public func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
        if change != nil, _device != nil {
            if keyPath == #keyPath(AVCaptureDevice.torchMode) {
                ...
                _flCameraEvent.sendEvent(["flash": state])
            } else if keyPath == #keyPath(AVCaptureDevice.videoZoomFactor) {
                var zoom = change![.newKey] as? Double
                if zoom == nil {
                    zoom = 1.0
                }
                _flCameraEvent.sendEvent(["zoomRatio": zoom, "maxZoomRatio": _device!.activeFormat.videoMaxZoomFactor])
            }
        }
    }
